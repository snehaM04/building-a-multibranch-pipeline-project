...
:: Uncomment the next line to install the dependency
...
:: Run npm test with the --passWithNoTests flag to prevent failure if no tests are found
npm test -- --passWithNoTests

:: Check if npm test failed and print appropriate message
IF %ERRORLEVEL% NEQ 0 (
    echo Tests failed!
    exit /b %ERRORLEVEL%
) ELSE (
